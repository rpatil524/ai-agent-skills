...
1. **RDF source of truth** — parse or generate the companion RDF first; derive ALL narrative content from that RDF, including: FAQ questions (`schema:Question`) and answers (`schema:Answer`), glossary terms (`schema:DefinedTerm`) and definitions, HowTo section (`schema:HowTo`) and every step (`schema:HowToStep`) with its exact `schema:name` and `schema:text`, person names, organization names, entity types, and KG Explorer graph data. Do not author HTML narrative content independently — any concept with an RDF entity representation MUST use that entity's data in the HTML. If the RDF has 7 `schema:HowToStep` entities, the HTML HowTo must render exactly 7 steps matching those entities.
